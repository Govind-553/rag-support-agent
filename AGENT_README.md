...
This project was developed with assistance from **Google Antigravity (Claude Sonnet 4.6)**:

- **Code generation:** Scaffolding for the RAG pipeline, orchestrator structure, and evaluation harness
- **Debugging:** Diagnosing the `device_map` → `accelerate` dependency issue
- **Example of an AI suggestion that was wrong:** The AI initially suggested `device_map="cpu"` for the transformers pipeline. This causes `ImportError: Using device_map requires Accelerate` when `accelerate` is not installed. The correct CPU argument is `device=-1`, which uses the standard PyTorch CPU device without requiring accelerate.

---

## Demo

> A 2–4 minute walkthrough showing:
> - Knowledge-base question with citations
> - Order lookup (ORD-1007)
> - Multi-turn follow-up (Canada shipping)
> - Agent abstention (vegan materials)
> - Evaluation suite running
>
> *(Record using `uvicorn app.main:app` + browser, or run `python evaluation/run_eval.py` for the eval demo)*
